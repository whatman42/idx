...
def apply_long_entry(state: PaperPortfolioState, *, symbol: str, price: float, weight: float,
    signal_id: str, timestamp: str, lot_size: float = DEFAULT_LOT_SIZE, fee_bps: float = SIM_FEE_BUY_BPS,
    slippage_bps: float = SIM_SLIPPAGE_BPS, tp: Optional[float] = None, sl: Optional[float] = None):
    ...
    # Different signal_id on same symbol is allowed (scale-in / average cost). Same signal_id blocked above.
    ...
    if existing:
        ...
        pos = PaperPosition(
            symbol=symbol, qty=total_qty, avg_entry=float(avg),
            entry_timestamp=existing.entry_timestamp or timestamp,
            signal_id=signal_id, side=1, last_mark=float(exec_px),
            tp=float(tp if tp else existing.tp), sl=float(sl if sl else existing.sl),
        )
    else:
...
def apply_exit(state: PaperPortfolioState, *, symbol: str, price: float, timestamp: str,
    reason: str = "MANUAL", fee_bps: float = SIM_FEE_EXIT_BPS, slippage_bps: float = SIM_SLIPPAGE_BPS):
...
def process_tp_sl_exits(state: PaperPortfolioState, marks: dict[str, float], timestamp: str,
    *, fee_bps: float = SIM_FEE_EXIT_BPS, slippage_bps: float = SIM_SLIPPAGE_BPS):
    closed = []
    ...
    *,
    ...
    for sym, pos in list(state.open_positions().items()):
        ...
        if sl_hit and tp_hit:
            # ASSUMPTION (INTRABAR_POLICY=SL_PRECEDENCE): not known which hit first
            reason = "SL_HIT"
