"""LLM is optional language layer only. Must not mutate structured trading facts."""
...
def validate_llm_output(text: str, report: CycleReport) -> list[str]:
    ...
    if report.signal and report.signal.decision == "NO_SIGNAL":
        if re.search(r"\bBUY\b", upper) and "NO SIGNAL" not in upper and "NO_SIGNAL" not in upper:
    ...
    if report.signal and report.signal.decision == "BUY":
        if re.search(r"🎯\s*SELL|DECISION:\s*SELL|\bSELL\s*[—\-]", upper):
...
def safe_compose(
    ...
    *,
    ...
    """Return (text, source) where source is 'deterministic' | 'llm' | 'deterministic_fallback'."""
